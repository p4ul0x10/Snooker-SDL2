	...
	while(count < 15){
		...
		if(count == 0){
			...
			if(count >= 8){
				...
				y_limit_space = count - 5;

				y_inc = y_inc + (1 * y_limit_space);
			}
...
void engine_show_all(SDL_Renderer *rendEngine, int n[], int m[], float v[], float f[], float x[], float y[], float ax[], float ay[], float ini_x[], float ini_y[]){
	...
	for(int m1 = 0; m1 <= 15; m1++){
		...
		if(x[m1] != 0 && y[m1] != 0 && m[m1] < 4){
			...
			if(m1 != m2 && n[m1] != n[m2] && x[m2] != 0 && y[m2] != 0){
				...
				if(check_x <= 20 && check_y <= 20){ //check_x & check_y limit for shock
					...
					if(m[m1] == 0){ // -y +x
						...
						if(conv_ang_x >= conv_ang_y){ // degs x > degs y
							...
							set_dest_xy(prox, limit_prox, m1, m2, px_wh, px_hw, m, x, y, ini_x, ini_y, ax, ay, v, f);
